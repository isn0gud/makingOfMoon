...
void ParticleSpriteRenderer::initFbos() {
    ...
    lumLod = (int) floor(log2((double) std::max(base_width, base_height) / 2));
...
ParticleSpriteRenderer::ParticleSpriteRenderer(int windowWidth, int windowHeight) {
    camera = new CameraRotateCenter(windowWidth, windowHeight);
    inputHandler = new SpriteRendererInputHandler(camera);
}

Camera_I *ParticleSpriteRenderer::getCamera() {
    return camera;
}

InputHandler_I *ParticleSpriteRenderer::getInputHandler() {
    return inputHandler;
}
